...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
    if (isset($_GET['type'])) {
        ...
        if ($_GET['type'] == 'get_antrian_sekarang') {
            ...
            if ($rows <> 0) {
                ...
                while ($row = mysqli_fetch_assoc($query)) {
                    ...
                    $data['name_antrian']	= $row["name_antrian"];
                    $data['comp_antrian']	= $row["comp_antrian"];
        ...
        if ($_GET['type'] == 'update_antrian') {
            $id = $_GET['id'];
			$loket = $_GET['loket'];
            $update = $action->updateAntrian($id,$loket);
            if ($update) {
                echo json_encode([
                    'success' => true,
        ...
        if ($_GET['type'] == 'create_panggilan') {
            ...
            $comp = $_GET['comp'];
            $loket = $_GET['loket'];
            $update = $action->createPanggilan($antrian, $nama, $comp, $loket);
            if ($update) {
                echo json_encode([
                    'success' => true,
