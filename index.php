...
<html lang="en" class="h-100">
	<head>
		...
		<meta name="description" content="Aplikasi Antrian EMS">
		<meta name="author" content="MBLGZT">
		<!-- Title -->
		<title>EMS Antrian</title>
		<!-- Favicon icon -->
	...
	<body class="d-flex flex-column h-100" <?= (($url == 'monitor') ? 'style="background-color:'.($data["warna_background"]?$data["warna_background"]:"#fffff"):'') .'"'?> >
		...
		<!-- Footer -->
		<footer class="footer mt-auto py-6" <?= ($url == 'monitor') ? 'style="height: 7vh;'.'background-color:'.($data['warna_accent']?$data['warna_accent']:'#fff').';color:'.($data['warna_text']?$data['warna_text']:'#fff').'"':''; ?>>
			<div class="container">
				<!-- copyright -->
				<div class="copyright text-center mb-2 mb-md-0">
					<marquee behavior="left" direction="left"><b><?= $data['running_text'] ? $data['running_text'] : ''; ?></b></marquee>
					&copy; <?= date('Y') ?> - <a href="https://www.seven-event.com/" target="_blank" class="text-brand text-decoration-none">SEVEN-EVENT.COM</a>. All rights reserved.
				</div>
			</div>
		</footer>
